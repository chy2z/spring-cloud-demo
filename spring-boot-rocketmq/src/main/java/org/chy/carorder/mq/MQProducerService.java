...
public class MQProducerService {
     ...
     */
    public void send(CarOrderEntity car) {
        rocketMQTemplate.send(topic + ":tag-event-one", MessageBuilder.withPayload(car)
                .setHeader("token", "chy")
                .setHeader("secret", "123456")
    ...
    public void sendAsyncMsg(String msgBody) {
        rocketMQTemplate.asyncSend(topic, MessageBuilder.withPayload(msgBody).build(), new SendCallback() {
            ...
            public void onSuccess(SendResult sendResult) {
                LOGGER.info("【sendAsyncMsg】onSuccess={}", JSON.toJSONString(sendResult));
            }
            @Override
            public void onException(Throwable throwable) {
                LOGGER.error("【sendAsyncMsg】onException", throwable);
            }
        });
    }
    ...
    public SendResult sendTagMsg(String msgBody) {
        return rocketMQTemplate.syncSend(topic + ":tag-event-two", MessageBuilder
                .withPayload(msgBody)
                .setHeader("token", "chy")
                .setHeader("secret", "123456")
                .setHeader(RocketMQHeaders.KEYS,System.currentTimeMillis())
     ...
     * 发送带tag的消息，直接在topic后面加上":tag"
     */
    public SendResult sendTagEntity(CarOrderEntity car) {
        return rocketMQTemplate.syncSend(topic + ":tag-event-two", MessageBuilder
                .withPayload(car)
                .setHeader("token", "chy")
                .setHeader("secret", "123456")
